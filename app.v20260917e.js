  ...
  (function () {
    ...
    function renderNiches() {
      ...
      $all('.niche-card').forEach(function (card) {
        if (card.getAttribute('data-click-bound')) return;
    ...
    function painsAcc(n) {
      ...
      var first = list.slice(0, 3).map(item).join('');
      var rest = list.slice(3);
      if (!rest.length) return first;
      return first +
        '<details class="pain-more"><summary>Показать ещё ' + rest.length + ' ' + (rest.length === 1 ? 'боль' : 'боли') + '</summary>' +
        '<div class="pain-more-in">' + rest.map(item).join('') + '</div></details>';
    }
